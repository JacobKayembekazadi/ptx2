  ...
  const navItems = [
    ...
    },
    {
      ...
      submenu: [
        ...
        { name: 'Fact Sheet', path: '/investors/fact-sheet' },
    ...
    <header className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-700 ${isScrolled ? 'bg-black/90 backdrop-blur-3xl border-b border-white/5 py-2' : 'bg-transparent py-5'}`}>
      <div className="container mx-auto px-6 flex justify-between items-center">
        <Link to="/" onClick={closeMobileMenu} className="relative group">
          <svg viewBox="0 0 600 136" className="w-32 sm:w-40 md:w-[200px] h-auto filter invert brightness-0">
          ...
          {navItems.map((item) => (
            <div key={item.name} className="relative group">
              {item.path ? (
                <Link
                  to={item.path}
                  className={`flex items-center px-5 py-3 rounded-full font-black text-[10px] uppercase tracking-[0.2em] transition-all hover:bg-brand-orange hover:text-white ${location.pathname === item.path ? 'bg-brand-orange text-white' : 'text-white'}`}
                >
                  {item.name}
                </Link>
              ) : (
                <button
                  className="flex items-center px-5 py-3 rounded-full font-black text-[10px] uppercase tracking-[0.2em] transition-all hover:bg-brand-orange hover:text-white text-white"
                >
                  {item.name}
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 bg-black z-[200] p-6 sm:p-8 md:p-12 overflow-y-auto">
          <div className="flex justify-between items-center mb-10 sm:mb-16 md:mb-20">
            <svg viewBox="0 0 600 136" className="w-32 sm:w-40 h-auto filter invert brightness-0">
              <image x="6" y="7" width="591" height="124" href="https://wp-platinex-2024.s3.ca-central-1.amazonaws.com/media/2024/04/07152334/ptx-metals.png" />
            </svg>
              ...
              <div key={item.name}>
                {item.submenu ? (
                  <button
                    ...
                    className="text-3xl sm:text-4xl md:text-5xl font-black uppercase tracking-tighter text-white hover:text-brand-orange transition-colors flex justify-between items-center w-full"
                  >
                    {item.name}
                  ...
                  <div className="mt-6 space-y-4 pl-4 border-l border-brand-orange">
                    {item.submenu.map(sub => (
                      <Link
                        ...
                        to={sub.path}
                        onClick={closeMobileMenu}
                        className="block text-base sm:text-lg md:text-xl font-bold uppercase tracking-widest text-gray-500 hover:text-white"
                      >
                        {sub.name}
                      </Link>
                    ))}
