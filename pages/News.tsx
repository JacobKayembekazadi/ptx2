...
import { Share2, ArrowLeft, Download, ChevronRight, Search } from 'lucide-react';
  ...
  return (
    <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-lg sticky top-32">
      ...
      <ul className="space-y-3">
        {years.map((year) => (
          <li key={year}>
            <button className="w-full text-left px-4 py-3 rounded-xl transition-all flex justify-between items-center group text-gray-600 hover:bg-gray-50 hover:text-brand-orange border border-transparent hover:border-gray-100">
...
export const NewsArticle: React.FC = () => {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50">
                <div className="text-center">
                    ...
                    <button onClick={() => navigate('/news')} className="text-brand-orange font-bold uppercase tracking-widest hover:underline">
                        Return to News
                    </button>
                </div>
            </div>
    ...
    return (
        <div className="bg-white min-h-screen">
          <div className="relative h-[60vh] min-h-[500px] w-full flex items-end pb-20 overflow-hidden">
            <div
              className="absolute inset-0 bg-cover bg-center transition-transform duration-1000 scale-105"
              style={{ backgroundImage: `url('${article.thumbnail || "https://wp-platinex-2024.s3.ca-central-1.amazonaws.com/media/2024/04/12092016/Photo-2024-04-06-3-06-48%E2%80%AFPM-scaled.jpg"}')` }}
            />
            ...
            <div className="container relative mx-auto px-6 z-10">
              ...
              <div className="max-w-4xl border-l-4 border-brand-orange pl-8 animate-fade-in-up">
                ...
                <h1 className="text-3xl md:text-5xl lg:text-6xl font-black text-white uppercase tracking-tighter leading-[1.1] drop-shadow-lg">
                  {article.title}
                </h1>
              </div>
            </div>
          ...
          <div className="container mx-auto px-6 py-20 flex flex-col lg:flex-row gap-16">
            <div className="lg:w-3/4">
              <div className="prose prose-lg prose-gray max-w-none font-light leading-relaxed">
                 <div className="font-serif text-lg text-gray-800" dangerouslySetInnerHTML={{ __html: article.content }} />
              </div>
                ...
                <h4 className="text-xl font-black uppercase tracking-tight mb-8">About PTX Metals</h4>
                 ...
                 <div className="bg-gray-50 p-8 rounded-2xl border border-gray-100">
                    <p className="text-xs text-gray-400 leading-relaxed">
                        <strong>Forward-Looking Information:</strong> This news release contains forward-looking information... (Standard Disclaimer)
                    </p>
                 </div>
              </div>
            </div>
            ...
            <div className="lg:w-1/4 space-y-8">
              <div className="bg-brand-dark p-8 rounded-2xl text-white shadow-xl sticky top-32">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button className="p-3 bg-white/10 rounded-lg hover:bg-brand-orange transition-colors text-xs font-bold uppercase tracking-wider">Twitter</button>
                  <button className="p-3 bg-white/10 rounded-lg hover:bg-brand-orange transition-colors text-xs font-bold uppercase tracking-wider">LinkedIn</button>
                  <button className="p-3 bg-white/10 rounded-lg hover:bg-brand-orange transition-colors text-xs font-bold uppercase tracking-wider">Facebook</button>
                  <button className="p-3 bg-white/10 rounded-lg hover:bg-brand-orange transition-colors text-xs font-bold uppercase tracking-wider">Email</button>
                </div>
              </div>
            </div>
          </div>
...
const News: React.FC = () => {
  ...
  // Use the designated 'Featured' article or just the first one
        ...
        <div className="container relative mx-auto px-6 z-10 pb-20">
          <div className="grid lg:grid-cols-2 gap-12 items-end">
            ...
            <div className="reveal-up">
              ...
              <p className="text-lg text-gray-300 font-light mb-8 line-clamp-3 max-w-xl leading-relaxed">
           ...
           <div className="grid grid-cols-1 gap-12">
             {filteredNews.length > 0 ? (
                 filteredNews.map((item, idx) => (
                    <div key={idx} className="bg-white p-8 rounded-3xl border border-gray-100 hover:shadow-xl transition-all group flex flex-col md:flex-row gap-8 items-start">
                        {item.thumbnail && (
                        ...
                        <div className="flex-1">
                            <div className="flex items-center space-x-3 mb-4">
                                ...
                                <span className="text-gray-400 font-bold text-xs uppercase tracking-widest">{item.year}</span>
                            </div>
                            <h3 className="text-2xl font-black uppercase tracking-tight text-gray-900 mb-4 leading-tight group-hover:text-brand-orange transition-colors">
                                <Link to={`/news/${item.id}`}>{item.title}</Link>
                            </h3>
                            <p className="text-gray-500 font-light mb-6 line-clamp-2">{item.summary}</p>
                            <Link to={`/news/${item.id}`} className="inline-flex items-center text-xs font-black uppercase tracking-[0.2em] text-black border-b-2 border-brand-orange pb-1 hover:text-brand-orange transition-colors">
        ...
        <div className="lg:w-1/4">
           ...
           <div className="mt-12 bg-gradient-to-br from-brand-orange to-red-600 p-8 rounded-2xl text-white shadow-xl relative overflow-hidden">
                ...
                <button 
                  ...
                  className="w-full bg-white text-brand-orange py-3 rounded-xl font-bold uppercase tracking-widest text-xs hover:bg-gray-50 transition-colors relative z-10 shadow-lg"
                >
                    Access Kit
                </button>
           </div>
